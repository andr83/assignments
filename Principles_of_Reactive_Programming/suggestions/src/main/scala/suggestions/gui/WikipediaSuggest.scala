...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = searchTerms
      .sanitized concatRecovered wikiSuggestResponseStream

    // TO IMPLEMENT
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(wikiSuggestions) => suggestionList.listData = wikiSuggestions
        case Failure(e) => status.text = e.getMessage
      }
    }

    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.flatMap(b => {
      suggestionList.selection.items.length match {
        case 0 => Observable.error(new Exception("Nothing selected"))
        case _ => Observable.just(suggestionList.selection.items.head)
      }
    })

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections
      .sanitized concatRecovered wikiPageResponseStream
