...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size <= minimumSize)
        ...
        val (lx, rx, ly, ry) = (centerX - size / 4, centerX + size / 4, centerY - size / 4, centerY + size / 4)
        val nw = Empty(lx, ly, size / 2)
        val sw = Empty(lx, ry, size / 2)
        val ne = Empty(rx, ly, size / 2)
        val se = Empty(rx, ry, size / 2)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      def normalize(value: Float, min: Float, max: Float): Int = {
        val boundedValue = if (value < min)
          min
        else if (value >= max)
          max - 1
        else value
        (((boundedValue - min) / (max - min)) * sectorPrecision).toInt
      }

      val sector = (normalize(b.y, boundaries.minY, boundaries.maxY) * sectorPrecision) + normalize(b.x, boundaries.minX, boundaries.maxX)
