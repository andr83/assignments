...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.isEmpty) {
      means.map(mp=> mp -> GenSeq.empty[Point]).toMap
    } else {
      points.map(p => findClosest(p, means) -> p).groupBy(_._1).mapValues(_.map(_._2))
    }
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map(oldP=> findAverage(oldP, classified.getOrElse(oldP, GenSeq.empty[Point])))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    ...
    while (i < oldMeans.length) {
      val oldP = oldMeans(i)
      val newP = newMeans(i)
      if (oldP.squareDistance(newP) > eta) {
        return false
      }
      i += 1
    }
